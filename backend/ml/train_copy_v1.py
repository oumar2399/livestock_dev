...
Target   : Behavior classification (13 fine-grained labels → 4 simplified states)
Method   : Random Forest with Leave-One-Animal-Out (LOAO) cross-validation
Output   : ml/models/behavior_classifier.pkl
...
-----------------
  ...
  5. Extract windows           — 5 s non-overlapping windows, 12 statistical features
...
WINDOW_SECONDS  = 5                              # how long each window lasts (seconds)
WINDOW_SAMPLES  = TARGET_FREQ * WINDOW_SECONDS   # 10 Hz × 5 s = 50 samples per window
...
# Within a 50-sample window, at least 80% of the samples must share the
...
#   35 samples "WAL" + 15 samples "STA"  →  35/50 = 70%  < 80%  → discarded
#
# Example of an ACCEPTED window (purity = 84%):
#   42 samples "LIE" + 8 samples "BLN"   →  42/50 = 84%  ≥ 80%  → kept as "lying"
LABEL_PURITY_THRESHOLD = 0.80

# ── Behavior label mapping ────────────────────────────────────────────────────
# The Japanese dataset uses up to 13 fine-grained annotation codes.
# We collapse them into 4 simplified states that the mobile app displays
# and that are meaningful to farmers and veterinarians.
#
...
BEHAVIOR_MAP: dict[str, str] = {
    ...
    # ── Lying (Couché) ────────────────────────────────────────────────────────
    # "REL" est votre SEULE source pour cette classe (764 lignes seulement).
    "REL": "lying",      # Resting in lying position

    # ── Standing (Statique / Debout) ──────────────────────────────────────────
    "RES": "standing",   # Resting in standing position (Le plus gros bloc après ETC)
    "RUS": "standing",   # Ruminating in standing position (C'est du statique !)
    "GRZ": "standing",   # Grazing
    "FES": "standing",   # Feeding in stancheon (Manger à l'auge)
    "DRN": "standing",   # Drinking
    "SLT": "standing",   # Salt licking
    "LCK": "standing",   # Licking
    "URI": "standing",   # Urinating
    "BMN": "standing",   # Being mounted (La vache qui subit reste immobile)

    # ── Walking (Marche) ──────────────────────────────────────────────────────
    "MOV": "walking",    # Moving (Marche classique)

    # ── Running (Urgence / Action forte) ──────────────────────────────────────
    "ATT": "running",    # Attacking (Mouvement violent)
    "ESC": "running",    # Escaping (Fuite)
}

# The 4 simplified behavior states that appear in the mobile app and in the
# model's output. The order here defines the LabelEncoder's integer mapping
# (alphabetical → lying=0, running=1, standing=2, walking=3) and must stay
# consistent between training and inference.
SIMPLIFIED_STATES: list[str] = ["lying", "standing", "walking", "running"]
...
def downsample_cow(group: pd.DataFrame, animal_id: str) -> pd.DataFrame:
    ...
    would cause a systematic feature mismatch (e.g. std computed over 125 samples
    vs. 50 samples produces fundamentally different values for the same behavior).
...
def extract_window_features(window: pd.DataFrame) -> dict:
    """
    Compute 12 statistical features from one 50-sample (5-second) window.
...
def apply_windowing(df: pd.DataFrame) -> pd.DataFrame:
    """
    Slide a non-overlapping 5-second window over each continuous segment and
    extract 12 statistical features from every accepted window.

    Why non-overlapping windows?
    ----------------------------
    Overlapping windows (e.g. stride = 25 samples) would roughly double the
    ...
    Returns
    -------
    pd.DataFrame where each row = one accepted 5-second window.
    ...
    for (animal_id, seg_id), group in df.groupby(["animal_id", "segment_id"]):
        ...
        # Integer division: only create complete 50-sample windows.
        ...
        for i in range(n_windows):
            ...
            # purity         = fraction of the 50 samples that share that label.
...
def loao_cross_validation(
    ...
    # Specific check for the "running" class, which is rarest by nature.
    # .get() returns a zero-filled Series if the column does not exist at all
    # (e.g. BEHAVIOR_MAP has no "running" entries and zero windows were created).
    running_col       = pivot.get("running", pd.Series(0, index=pivot.index))
    n_without_running = (running_col == 0).sum()
    if n_without_running > 0:
        logger.warning(
            f"'running' class has zero windows for {n_without_running} animal(s). "
            f"LOAO folds where the TEST animal has no running data "
            f"will not evaluate running recall — each such fold is flagged below.\n"
            f"  → If running is absent for most animals, consider merging it "
            f"into 'walking' to avoid training an under-represented class."
        )
    ...
    # Tests whether per-fold balanced accuracy values are significantly above 0.25
    # (chance level for 4 classes). Non-parametric — appropriate for n=6 folds
    # where normality cannot be assumed.
    #
    # H0: median fold accuracy = 0.25 (model performs at chance)
    # H1: median fold accuracy > 0.25 (model learns something)
    #
    ...
    if n_folds >= 4:
        ...
        wilcoxon_str = (
            f"Wilcoxon signed-rank test vs chance (0.25): "
...
def save_model(
    ...
    window_samples / target_freq
        ...
        "I expect 50 samples at 10 Hz — if you send 25, something is wrong."
    ...
    artifact = {
        ...
        "window_samples": WINDOW_SAMPLES,   # expected: 50 samples per window
...
def main() -> None:
    ...
    # ── Step 2b: Remove corrupted sensor data  ← AJOUTER CETTE LIGNE
    ...
    if len(df_windows) == 0:
        raise RuntimeError(
            ...
            "  4. Recording files are too short to form even one 50-sample window\n"
        )
    ...
    output_path = MODEL_DIR / "behavior_classifier.pkl"
