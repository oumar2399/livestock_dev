...
def apply_windowing(
    ...
    """
    logger.info(
        f"Windowing: {window_samples} samples = {window_samples / TARGET_FREQ:.0f} s, "
        f"purity threshold = {purity_threshold:.0%} …"
    )
...
def preprocess_to_10hz(data_dir: Path = DATA_DIR) -> pd.DataFrame:
    """
    Run Steps 1–3 only: load raw CSVs, clean, normalize labels, downsample to 10 Hz.

    This is exactly the part of the pipeline that does NOT depend on
    window_seconds or purity_threshold. Factored out so that scripts like
    ablation_study.py can compute it ONCE and reuse it across many
    (window_seconds, purity_threshold) combinations, instead of paying the
    downsampling cost (the slowest step — ~2 min on the full dataset) on
    every single run_pipeline() call.

    Returns
    -------
    pd.DataFrame at 10 Hz, ready to be passed as `df_10hz=` to run_pipeline().
    """
    ...
    return downsample_dataset(df_raw)


def run_pipeline(
    window_seconds: int = WINDOW_SECONDS,
    purity_threshold: float = LABEL_PURITY_THRESHOLD,
    save_artifact: bool = False,
    df_10hz: pd.DataFrame | None = None,
) -> dict:
    """
    ...
    Parameters
    ----------
    df_10hz : optional pre-computed 10 Hz dataframe (from preprocess_to_10hz()).
        If provided, Steps 1–3 (load / clean / normalize / downsample) are
        SKIPPED and this dataframe is used directly. This is the mechanism
        that lets ablation_study.py avoid redundant downsampling across grid
        points — window_seconds and purity_threshold only affect Step 4
        onward, so Steps 1–3 never need to be repeated within one ablation run.
        If None (default), the full pipeline runs from raw CSVs as before —
        this keeps single-call usage (e.g. the non-regression test, or
        `python -m ml.train`) unchanged.
    ...
    if df_10hz is None:
        ...
        logger.info("Steps 1–3 skipped — using precomputed 10 Hz dataframe.\n")

    # ── Step 4: Sliding window + feature extraction ───────────────────────────
    window_samples = TARGET_FREQ * window_seconds
    df_windows = apply_windowing(
        df_10hz,
    ...
    if save_artifact:
        output_path = MODEL_DIR / "behavior_classifier2.pkl"
        save_model(clf, label_encoder, loao_metrics, output_path)
        logger.info("✓ Training pipeline complete.")

    # ── Volume de fenêtres par classe, pour le tableau d'ablation ────────────
    class_counts = df_windows["label_simplified"].value_counts().to_dict()

    return {
        "window_seconds": window_seconds,
        "purity_threshold": purity_threshold,
        "n_windows_total": len(df_windows),
        "n_windows_active": class_counts.get("Active", 0),
        "n_windows_resting": class_counts.get("Resting", 0),
        "loao_metrics": loao_metrics,
    }


if __name__ == "__main__":
    result_15s = run_pipeline(window_seconds=15, purity_threshold=0.70)
    print(result_15s['n_windows_total'], result_15s['loao_metrics']['overall_balanced_accuracy'])