...
def run(config, max_cycles=None, bench_clock=None, on_status=None):
    ...
    def poll_gps():
        ...
        if is_bench_mode:
            return
        if uart.any():
            gps.feed(uart.read(min(uart.any(), 256)) or b"", time.ticks_ms())
    ...
    while max_cycles is None or cycle_count < max_cycles:
        ...
        try:
            for _ in range(150):
                while time.ticks_diff(deadline, time.ticks_ms()) > 0:
                    poll_gps()
                    time.sleep_ms(1)
                if time.ticks_diff(time.ticks_ms(), deadline) > jitter:
                    raise ValueError("Sampling deadline missed")
                window.add(tuple(value / gravity for value in imu.acceleration()))
        ...
        if time.ticks_diff(end_tick, deadline) > jitter:
            invalid_window()
            continue
